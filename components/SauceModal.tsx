...
const SauceModal: React.FC<SauceModalProps> = ({ product, onClose, onConfirm, initialSalsas = [] }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-[102] p-4 font-sans" onClick={onClose}>
            <div className="bg-surface dark:bg-zinc-800 rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col animate-fade-in-scale border border-text-primary/10 dark:border-zinc-700" onClick={e => e.stopPropagation()}>
                <div className="p-6 border-b border-text-primary/10 dark:border-zinc-700">
                    <h2 className="text-2xl font-heading font-bold text-text-primary dark:text-zinc-100">Añade tus cremas para</h2>
                    <p className="text-primary dark:text-orange-400 font-semibold text-lg">{product.nombre}</p>
                </div>
                <div className="p-6 space-y-4 overflow-y-auto">
                    <h3 className="font-semibold text-text-secondary dark:text-zinc-400">Elige tus favoritas:</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {availableSalsas.map(sauce => (
                            <label key={sauce.nombre} className="flex items-center space-x-3 bg-background dark:bg-zinc-700/50 p-3 rounded-lg cursor-pointer hover:bg-text-primary/5 dark:hover:bg-zinc-700 transition-colors border border-text-primary/10 dark:border-zinc-600 has-[:checked]:bg-primary/10 has-[:checked]:border-primary/50">
                                <input
                                    type="checkbox"
                                    checked={selectedSalsas.some(s => s.nombre === sauce.nombre)}
                                    onChange={() => handleSauceToggle(sauce)}
                                    className="h-5 w-5 rounded border-text-primary/20 dark:border-zinc-500 text-primary focus:ring-primary bg-transparent dark:bg-zinc-800"
                                />
                                <span className="flex-grow text-text-primary dark:text-zinc-200 font-medium">{sauce.nombre}</span>
                ...
                <div className="p-6 border-t border-text-primary/10 dark:border-zinc-700 mt-auto bg-background dark:bg-zinc-900/50 rounded-b-2xl">
                    <div className="flex justify-between items-center mb-4">
                        <span className="text-lg font-heading font-bold text-text-primary dark:text-zinc-100">Total Producto:</span>
