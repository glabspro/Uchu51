...
const SauceModal: React.FC<SauceModalProps> = ({ product, onClose, onConfirm }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-[100] p-4 font-sans" onClick={onClose}>
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col animate-fade-in-scale" onClick={e => e.stopPropagation()}>
                <div className="p-6 border-b border-stone-200">
                    <h2 className="text-2xl font-bold text-stone-800">Añade tus cremas para</h2>
                    <p className="text-primary font-semibold text-lg">{product.nombre}</p>
                </div>
                <div className="p-6 space-y-4 overflow-y-auto">
                    <h3 className="font-semibold text-stone-600">Elige tus favoritas:</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {listaDeSalsas.map(sauce => (
                            <label key={sauce.nombre} className="flex items-center space-x-3 bg-stone-100 p-3 rounded-lg cursor-pointer hover:bg-stone-200 transition-colors border border-transparent has-[:checked]:bg-primary/10 has-[:checked]:border-primary/50">
                                <input
                                    type="checkbox"
                                ...
                                />
                                <span className="flex-grow text-stone-800 font-medium">{sauce.nombre}</span>
                ...
                <div className="p-6 border-t mt-auto bg-stone-50 rounded-b-2xl">
                    <div className="flex justify-between items-center mb-4">
                        <span className="text-lg font-bold text-stone-800">Total Producto:</span>
