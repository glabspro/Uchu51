                ...
                {mesas.map((mesa, i) => (
                    ...
                    >
                        ...
                        <span className={`mt-4 text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full ${
                            mesa.ocupada ? 'bg-primary/10 text-primary' : 'bg-text-primary/10 dark:bg-slate-700 text-text-primary dark:text-slate-200'
                        }`}>
                            {mesa.ocupada ? 'Ocupada' : 'Libre'}
                        </span>
                        {mesa.ocupada && (
                            <span className="mt-2 text-xs font-mono text-text-secondary/60 dark:text-slate-500">{mesa.pedidoId}</span>
                        )}
                    </button>
                ))}
            </div>
