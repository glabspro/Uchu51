    ...
    return (
        <div>
            ...
            {showDeleteConfirm && (
                 <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[101] p-4">
                    <div className="bg-surface dark:bg-zinc-800 rounded-2xl shadow-xl p-6 max-w-sm w-full text-center animate-fade-in-scale">
                        ...
                        <p className="text-text-secondary dark:text-zinc-400 my-3">¿Estás seguro de que quieres eliminar <span className="font-bold">{showDeleteConfirm.nombre}</span>? Esta acción no se puede deshacer.</p>
                        <div className="grid grid-cols-2 gap-3 mt-6">
                            <button onClick={() => setShowDeleteConfirm(null)} className="bg-text-primary/10 dark:bg-zinc-700 hover:bg-text-primary/20 dark:hover:bg-zinc-600 text-text-primary dark:text-zinc-200 font-bold py-2 px-4 rounded-lg transition-colors active:scale-95">Cancelar</button>
            ...
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-text-primary dark:text-zinc-200">Menú de Productos ({products.length})</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 max-h-[calc(100vh-22rem)] overflow-y-auto pr-3">
                {products.map(product => (
                    <div key={product.id} className="bg-background dark:bg-zinc-900/50 rounded-xl overflow-hidden shadow-sm border border-text-primary/5 dark:border-zinc-700 flex flex-col">
                        <img src={product.imagenUrl} alt={product.nombre} className="w-full h-40 object-cover" />
                        <div className="p-4 flex-grow flex flex-col">
                            <h3 className="font-bold text-lg text-text-primary dark:text-zinc-100">{product.nombre}</h3>
                            <p className="text-sm text-text-secondary dark:text-zinc-400">{product.categoria}</p>
                            <div className="mt-2 pt-2 border-t border-text-primary/10 dark:border-zinc-700 flex-grow">
                                <div className="flex justify-between text-sm">
                                    <span className="text-text-secondary dark:text-zinc-400">Precio:</span>
                                    <span className="font-bold text-text-primary dark:text-zinc-200">S/.{product.precio.toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-text-secondary dark:text-zinc-400">Costo:</span>
                                    <span className="font-bold text-text-primary dark:text-zinc-200">S/.{product.costo.toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-text-secondary dark:text-zinc-400">Stock:</span>
                                    <span className={`font-bold ${product.stock < 10 ? 'text-danger' : 'text-text-primary dark:text-zinc-200'}`}>{product.stock}</span>
                                </div>
                            </div>
                        </div>
                        <div className="bg-background/50 dark:bg-zinc-800/50 p-2 grid grid-cols-2 gap-2">
