    ...
    return (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[102] p-4" onClick={onClose}>
            <div className="bg-surface dark:bg-zinc-800 rounded-2xl shadow-xl p-6 max-w-lg w-full flex flex-col animate-fade-in-scale" onClick={e => e.stopPropagation()}>
                <h2 className="text-2xl font-heading font-bold text-text-primary dark:text-zinc-100 mb-6">{reward ? 'Editar Recompensa' : 'Añadir Nueva Recompensa'}</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-text-secondary dark:text-zinc-400 mb-1">Producto Asociado (Opcional)</label>
                        <select name="productoId" value={formData.productoId || 'none'} onChange={handleChange} className="w-full bg-background dark:bg-zinc-700 border border-text-primary/10 dark:border-zinc-600 rounded-md p-2">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-text-secondary dark:text-zinc-400 mb-1">Nombre de la Recompensa</label>
                        <input type="text" name="nombre" value={formData.nombre} onChange={handleChange} placeholder="Ej: Gaseosa Personal Gratis" className="w-full bg-background dark:bg-zinc-700 border border-text-primary/10 dark:border-zinc-600 rounded-md p-2" required />
                    </div>
                     <div>
                        <label className="block text-sm font-medium text-text-secondary dark:text-zinc-400 mb-1">Puntos Requeridos</label>
                        <input type="number" name="puntosRequeridos" value={formData.puntosRequeridos} onChange={handleChange} placeholder="Ej: 50" className="w-full bg-background dark:bg-zinc-700 border border-text-primary/10 dark:border-zinc-600 rounded-md p-2" required />
                    </div>
                </form>
