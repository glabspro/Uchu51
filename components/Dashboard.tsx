...
const MetricCard: React.FC<{ title: string; value: string | number; }> = ({ title, value }) => (
    <div className="bg-surface dark:bg-slate-800 p-4 rounded-2xl shadow-lg border border-text-primary/5 dark:border-slate-700">
        ...
        <div className="bg-surface dark:bg-slate-800 p-6 rounded-2xl shadow-lg border border-text-primary/5 dark:border-slate-700 h-full">
            ...
            <div className="space-y-3">
                {recentOrders.length > 0 ? recentOrders.map((order, i) => (
                    ...
                    <p className="text-sm text-center text-text-secondary dark:text-slate-500 py-8">No hay actividad reciente en este período.</p>
                )}
            </div>
        </div>
    );
    ...
    return (
        <div className="bg-surface dark:bg-slate-800 p-6 rounded-2xl shadow-lg border border-text-primary/5 dark:border-slate-700">
            <div className="flex flex-wrap justify-between items-center gap-4 mb-4">
                <h3 className="text-xl font-heading font-bold text-text-primary dark:text-slate-100 flex items-center gap-2">
        ...
        <div className="space-y-6">
            <div className="bg-surface dark:bg-slate-800 p-4 rounded-2xl shadow-lg border border-text-primary/5 dark:border-slate-700 space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                    <div className="col-span-2">
                        <label className="text-xs font-semibold text-text-secondary dark:text-slate-400">Rango de Fechas</label>
                        <div className="flex items-center gap-2">
                            <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full bg-background dark:bg-slate-700/50 border border-text-primary/10 dark:border-slate-700 rounded-md p-2 text-sm"/>
                            <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full bg-background dark:bg-slate-700/50 border border-text-primary/10 dark:border-slate-700 rounded-md p-2 text-sm"/>
                        </div>
                    </div>
                     <div className="col-span-2 md:col-span-2 lg:col-span-2">
                        <label className="text-xs font-semibold text-text-secondary dark:text-slate-400">Filtros Rápidos</label>
                        <div className="flex flex-wrap gap-2">
                            <button onClick={() => setDateRange(today, today)} className="text-xs font-semibold bg-text-primary/10 dark:bg-slate-700/50 hover:bg-text-primary/20 dark:hover:bg-slate-600 px-2.5 py-1.5 rounded-md transition-colors flex-grow">Hoy</button>
                            <button onClick={() => { const y = new Date(); y.setDate(y.getDate()-1); setDateRange(y,y);}} className="text-xs font-semibold bg-text-primary/10 dark:bg-slate-700/50 hover:bg-text-primary/20 dark:hover:bg-slate-600 px-2.5 py-1.5 rounded-md transition-colors flex-grow">Ayer</button>
                            <button onClick={() => setDateRange(last7Days, today)} className="text-xs font-semibold bg-text-primary/10 dark:bg-slate-700/50 hover:bg-text-primary/20 dark:hover:bg-slate-600 px-2.5 py-1.5 rounded-md transition-colors flex-grow">7 Días</button>
                            <button onClick={() => setDateRange(firstDayOfMonth, today)} className="text-xs font-semibold bg-text-primary/10 dark:bg-slate-700/50 hover:bg-text-primary/20 dark:hover:bg-slate-600 px-2.5 py-1.5 rounded-md transition-colors flex-grow">Mes</button>
                            <button onClick={() => setDateRange(firstDayOfLastMonth, lastDayOfLastMonth)} className="text-xs font-semibold bg-text-primary/10 dark:bg-slate-700/50 hover:bg-text-primary/20 dark:hover:bg-slate-600 px-2.5 py-1.5 rounded-md transition-colors flex-grow">Mes Pasado</button>
                        </div>
                    </div>
                     <div>
                        <label className="text-xs font-semibold text-text-secondary dark:text-slate-400">Turno</label>
                        <select value={filterTurno} onChange={e => setFilterTurno(e.target.value as any)} className="w-full bg-background dark:bg-slate-700/50 border border-text-primary/10 dark:border-slate-700 rounded-md p-2 text-sm">
                    ...
                    <div>
                        <label className="text-xs font-semibold text-text-secondary dark:text-slate-400">Tipo Pedido</label>
                         <select value={filterTipo} onChange={e => setFilterTipo(e.target.value as any)} className="w-full bg-background dark:bg-slate-700/50 border border-text-primary/10 dark:border-slate-700 rounded-md p-2 text-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                 <div className="bg-surface dark:bg-slate-800 p-6 rounded-2xl shadow-lg border border-text-primary/5 dark:border-slate-700">
                    <h3 className="text-lg font-heading font-bold text-text-primary dark:text-slate-100 mb-4">Ventas en el Tiempo</h3>
                    <ResponsiveContainer width="100%" height={300}>
                        <LineChart data={salesOverTimeData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                            ...
                            <Tooltip contentStyle={{ backgroundColor: 'var(--tw-bg-surface)', border: '1px solid #e7e5e4', borderRadius: '12px' }} wrapperClassName="dark:!bg-slate-800 dark:!border-slate-600" />
                            <Line type="monotone" dataKey="sales" name="Ventas" stroke="#F97316" strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 8 }} />
                        </LineChart>
                    </ResponsiveContainer>
                </div>
                <div className="bg-surface dark:bg-slate-800 p-6 rounded-2xl shadow-lg border border-text-primary/5 dark:border-slate-700">
                    <h3 className="text-lg font-heading font-bold text-text-primary dark:text-slate-100 mb-4">Ventas por Categoría</h3>
                    <ResponsiveContainer width="100%" height={300}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                <div className="lg:col-span-3 bg-surface dark:bg-slate-800 p-6 rounded-2xl shadow-lg border border-text-primary/5 dark:border-slate-700">
                 ...
                 <div className="lg:col-span-2 bg-surface dark:bg-slate-800 p-6 rounded-2xl shadow-lg border border-text-primary/5 dark:border-slate-700">
                    <h3 className="text-lg font-heading font-bold text-text-primary dark:text-slate-100 mb-4">Ventas por Tipo de Pedido</h3>
                     <ResponsiveContainer width="100%" height={250}>
                        <PieChart>
                            <Pie data={salesByTypeData} cx="50%" cy="50%" labelLine={false} outerRadius={100} fill="#8884d8" dataKey="value" nameKey="name" label={({ name, percent }) => `${name} ${((percent || 0) * 100).toFixed(0)}%`} stroke="var(--tw-bg-surface)" className="dark:stroke-slate-800">
